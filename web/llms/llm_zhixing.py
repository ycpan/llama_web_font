...
from .get_embedding import compute_simility_score
...
def exec_step(current_plan,zhishiku,chanyeku,current_bak_data=''):
    try:
        ...
        if '数据库' in solution_type:
            ...
            solution_output = solution_output.replace('order by `产业评分`', 'order by `综合评分`')
...
含烟台新能源现有企业规模,分布,以及龙头企业等'}
        ...
        answer,plan_data = execution(current_output,prompt,history_data,zhishiku,chanyeku,init_question)
